...
def run_API_1 (departure_code, arrival_code, dep_date_input, ret_date_input):
    ...
    querystring = {"departureId": departure_code,
    "arrivalId": arrival_code,
    "departureDate": dep_date_input,
    "arrivalDate": ret_date_input}
...
def check_airport(departure_code, arrival_code, dep_date_input, ret_date_input):
    if departure_code and arrival_code:
        result = run_API_1(departure_code, arrival_code, dep_date_input, ret_date_input)
        return result
    else:
...
def get_df(API_results):
    data = pd.json_normalize(API_results)
    if len(data['data.topFlights']) < 1:
        print("There are no flights available for these cities and date.")
    else:
        df_0 = normalize(data['data.topFlights'])
        df_1 = normalize(df_0[0])
        df_2 = normalize(df_0[1])
        df_3 = normalize(df_0[2])
        df_4 = pd.concat([df_1, df_2])
        df_5 = pd.concat([df_3, df_4])
        df_6 = df_5.drop(columns=['segments', 'returningToken', 'isCodeShare', 'hasTransit', 'transferAirports', 'fareId', 'metaData', 'baggage', 'airline', 'isAvailable'])
        df_7 = df_6.sort_values(by="price")
